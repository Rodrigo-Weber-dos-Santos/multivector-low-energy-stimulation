...
void solve_Forward_Euler_cpu_new (Real time, Real *vm, Real *sv, Real dt, int NEQ, int cellID, int col);
void solve_Runge_Kutta_cpu       (Real time, Real *sv, Real dt, int NEQ, int cellID, int col);
void RHS_Luo_Rudy_1991_cpu       (Real time, Real *rY, Real *rDY, int col);
void RHS_Luo_Rudy_1991_pe_cpu    (Real time, Real *rY, Real *rDY, int col);
...
// Setting initial conditions to the Luo-Rudy 1991 model
void setIC_ode_cpu_new(Real *vm, Real *sv, int cellID, int NEQ = 8)
{
   // Vector containing the initial conditions of the cell
   Real IC[NEQ];
...
void solve_ode_cpu (Real time, Real dt, Real *sv, int cellID, int NEQ, int col)
{
   ...
   //solve_Runge_Kutta_cpu(time, sv, dt, NEQ, cellID, col);
}

// Solving the Luo-Rudy 1991 model for each cell in the tissue matrix ni x nj
void solve_Luo_Rudy_1991_cpu_new(Real time, Real dt, Real *vm, Real *sv, int cellID, int NEQ, int col)
{
  solve_Forward_Euler_cpu_new(time, vm, sv, dt, NEQ, cellID, col);
   //solve_Runge_Kutta_cpu(time, sv, dt, NEQ, cellID, col);
}
...
void solve_Forward_Euler_cpu(Real time, Real *sv, Real dt, int NEQ, int cellID, int col)
{
   ...
   RHS_Luo_Rudy_1991_pe_cpu(time, rY, rDY, col);
...
void solve_Forward_Euler_cpu_new(Real time, Real *vm, Real *sv, Real dt, int NEQ, int cellID, int col)
{
   ...
   vm[index] = dt*rDY[0] + rY[0];
   for(int i = 0; i < NEQ-1; i++)
      sv[cellID+i] = dt*rDY[i+1] + rY[i+1];
}
...
void solve_Runge_Kutta_cpu(Real time, Real *sv, Real dt, int NEQ, int cellID, int col)
{
   ...
   Real rY[NEQ], k1[NEQ], k2[NEQ], k3[NEQ], k4[NEQ];
   Real htime;

   // evaluate k1
   for(int i = 0; i < NEQ; i++)
      rY[i] = sv[cellID+i];
   RHS_Luo_Rudy_1991_cpu(time, rY, k1, col);

   // evaluate k2
   htime = time + dt/2.;
   for(int i = 0; i < NEQ; i++)
      rY[i] = sv[cellID+i] + (dt/2.)*k1[i];
   RHS_Luo_Rudy_1991_cpu(htime, rY, k2, col);

   // evaluate k3
   for(int i = 0; i < NEQ; i++)
      rY[i] = sv[cellID+i] + (dt/2.)*k2[i];
   RHS_Luo_Rudy_1991_cpu(htime, rY, k3, col);

   // evaluate k4
   htime = time + dt;
   for(int i = 0; i < NEQ; i++)
      rY[i] = sv[cellID+i] + dt*k3[i];
   RHS_Luo_Rudy_1991_cpu(htime, rY, k4, col);

   // evaluate the slope
   for(int i = 0; i < NEQ; i++)
      sv[cellID+i] = sv[cellID+i] + (dt/6.)*(k1[i] + 2.*k2[i] + 2.*k3[i] + k4[i]);
}
...
void RHS_Luo_Rudy_1991_cpu(Real time, Real *rY, Real *rDY, int col)
{
   ...
   Real var_time_dependent_potassium_current_Xi_gate__Xi = (var_time_dependent_potassium_current_Xi_gate__V > (-100.0)) ? ((2.837 * (exp(0.04 * (var_time_dependent_potassium_current_Xi_gate__V + 77.0)) - 1.0)) / ((var_time_dependent_potassium_current_Xi_gate__V + 77.0) * exp(0.04 * (var_time_dependent_potassium_current_Xi_gate__V + 35.0)))) : 1.0;
   ...
   Real var_fast_sodium_current_m_gate__alpha_m = (0.32 * (var_fast_sodium_current_m_gate__V + 47.13)) / (1.0 - exp((-0.1) * (var_fast_sodium_current_m_gate__V + 47.13)));
...
void RHS_Luo_Rudy_1991_pe_cpu(Real time, Real *rY, Real *rDY, int col)
{
   ...
   // Mathematics
   Real var_membrane__i_Na = 23.0 * pow(var_fast_sodium_current_m_gate__m, 3.0) * var_fast_sodium_current_h_gate__h * var_fast_sodium_current_j_gate__j * (var_membrane__V - 54.7944639351);
   Real var_slow_inward_current__i_si = 0.09 * var_slow_inward_current_d_gate__d * var_slow_inward_current_f_gate__f * (var_membrane__V - (7.7 - (13.0287 * log(var_intracellular_calcium_concentration__Cai * 1.0))));
   Real var_membrane__i_si = var_slow_inward_current__i_si;

  // Avoid NaN
   Real var_membrane__i_K;
   if (var_membrane__V == -77.0){
     var_membrane__i_K = 0.6089;
   } else {
     var_membrane__i_K = 0.282 * var_time_dependent_potassium_current_X_gate__X * ((var_membrane__V >  -100.0) ? ((2.837 * (exp(0.04 * (var_membrane__V + 77.0)) - 1.0)) / ((var_membrane__V + 77.0) * exp(0.04 * (var_membrane__V + 35.0)))) : 1.0) * (var_membrane__V -  -77.5675843853);
   }

   Real var_time_independent_potassium_current_K1_gate__alpha_K1 = 1.02 / (1.0 + exp(0.2385 * ((var_membrane__V -  -87.8929017138) - 59.215)));
   Real var_membrane__i_K1 = 0.6047 * (var_time_independent_potassium_current_K1_gate__alpha_K1 / (var_time_independent_potassium_current_K1_gate__alpha_K1 + (((0.49124 * exp(0.08032 * ((var_membrane__V + 5.476) -  -87.8929017138))) + (1.0 * exp(0.06175 * (var_membrane__V - 506.417098286)))) / (1.0 + exp( -0.5143 * ((var_membrane__V -  -87.8929017138) + 4.753)))))) * (var_membrane__V -  -87.8929017138);
   Real var_membrane__i_Kp = 0.0183 * (1.0 / (1.0 + exp((7.488 - var_membrane__V) * 0.167224080268))) * (var_membrane__V -  -87.8929017138);
   Real var_membrane__i_b = 0.03921 * (var_membrane__V -  -59.87);

   Real d_dt_membrane__V =  -1.0 * ((((var_environment__time >= 100.0) && (var_environment__time <= 100000000000.0) && (((var_environment__time - 100.0) - (floor((var_environment__time - 100.0) * 0.001) * 1000.0)) <= 2.0)) ?  0.0 : 0.0) + var_membrane__i_Na + var_membrane__i_si + var_membrane__i_K + var_membrane__i_K1 + var_membrane__i_Kp + var_membrane__i_b);

  // Avoid NaN
   Real d_dt_fast_sodium_current_m_gate__m;
   if(var_membrane__V == -47.13){
     d_dt_fast_sodium_current_m_gate__m = 3.2;
   } else {
     d_dt_fast_sodium_current_m_gate__m = (((0.32 * (var_membrane__V + 47.13)) / (1.0 - exp( -0.1 * (var_membrane__V + 47.13)))) * (1.0 - var_fast_sodium_current_m_gate__m)) - ((0.08 * exp((-var_membrane__V) * 0.0909090909091)) * var_fast_sodium_current_m_gate__m);
   }

   Real d_dt_fast_sodium_current_h_gate__h = (((var_membrane__V <  -40.0) ? (0.135 * exp((80.0 + var_membrane__V) *  -0.147058823529)) : 0.0) * (1.0 - var_fast_sodium_current_h_gate__h)) - (((var_membrane__V <  -40.0) ? ((3.56 * exp(0.079 * var_membrane__V)) + (310000.0 * exp(0.35 * var_membrane__V))) : (1.0 / (0.13 * (1.0 + exp((var_membrane__V + 10.66) *  -0.0900900900901))))) * var_fast_sodium_current_h_gate__h);
   Real d_dt_fast_sodium_current_j_gate__j = (((var_membrane__V <  -40.0) ? (((( -127140.0 * exp(0.2444 * var_membrane__V)) - (3.474e-05 * exp( -0.04391 * var_membrane__V))) * (var_membrane__V + 37.78)) / (1.0 + exp(0.311 * (var_membrane__V + 79.23)))) : 0.0) * (1.0 - var_fast_sodium_current_j_gate__j)) - (((var_membrane__V <  -40.0) ? ((0.1212 * exp( -0.01052 * var_membrane__V)) / (1.0 + exp( -0.1378 * (var_membrane__V + 40.14)))) : ((0.3 * exp( -2.535e-07 * var_membrane__V)) / (1.0 + exp( -0.1 * (var_membrane__V + 32.0))))) * var_fast_sodium_current_j_gate__j);
   Real d_dt_slow_inward_current_d_gate__d = (((0.095 * exp( -0.01 * (var_membrane__V - 5.0))) / (1.0 + exp( -0.072 * (var_membrane__V - 5.0)))) * (1.0 - var_slow_inward_current_d_gate__d)) - (((0.07 * exp( -0.017 * (var_membrane__V + 44.0))) / (1.0 + exp(0.05 * (var_membrane__V + 44.0)))) * var_slow_inward_current_d_gate__d);
   Real d_dt_slow_inward_current_f_gate__f = (((0.012 * exp( -0.008 * (var_membrane__V + 28.0))) / (1.0 + exp(0.15 * (var_membrane__V + 28.0)))) * (1.0 - var_slow_inward_current_f_gate__f)) - (((0.0065 * exp( -0.02 * (var_membrane__V + 30.0))) / (1.0 + exp( -0.2 * (var_membrane__V + 30.0)))) * var_slow_inward_current_f_gate__f);
   Real d_dt_time_dependent_potassium_current_X_gate__X = (((0.0005 * exp(0.083 * (var_membrane__V + 50.0))) / (1.0 + exp(0.057 * (var_membrane__V + 50.0)))) * (1.0 - var_time_dependent_potassium_current_X_gate__X)) - (((0.0013 * exp( -0.06 * (var_membrane__V + 20.0))) / (1.0 + exp( -0.04 * (var_membrane__V + 20.0)))) * var_time_dependent_potassium_current_X_gate__X);
   Real d_dt_intracellular_calcium_concentration__Cai = ( -0.0001 * var_slow_inward_current__i_si) + (0.07 * (0.0001 - var_intracellular_calcium_concentration__Cai));
