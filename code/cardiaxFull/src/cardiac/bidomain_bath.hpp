...
class BidomainBath : public CardiacProblem
{
public:
...
protected:
  ...
  */
  ...
  //void write_vtk_ve(const arma::vec & u, const std::string & s, int * step);

  //! Write VTU (VTK) files for tissue region only 
  //void write_vtk_vm(const arma::vec & u, const std::string & s, int * step);
